...
pub struct InitializeTreasury<'info> {
    ...
    /// CHECK: This is going to be the mint authority
...
pub struct CloseProposal <'info> {
    ...
    #[account(
        mut,
        seeds = [b"proposal", proposal_id.to_be_bytes().as_ref()],bump,
        close = destination,
        constraint = proposal_account.authority == authority.key() @ VoteError::UnauthorizedAccess
    )]
    pub proposal_account: Account<'info, Proposal>,

    /// CHECK: Destination account to receive refunded rent
    #[account(mut)]
...
pub struct WithdrawSol<'info> {
    #[account(
        seeds = [b"treasury_CA"], bump,
        constraint = treasury_config.authority == authority.key() @ VoteError::UnauthorizedAccess
    )]
    pub treasury_config: Account<'info, TreasuryConfig>,
    
    /// CHECK: This is a PDA vault that holds SOL tokens
    #[account(mut,
        seeds = [b"sol_vault"], bump = treasury_config.bump
    )]
    pub sol_vault: AccountInfo<'info>,
